...
Le PCWG3, publié en 2016 par Scher et al. [[2]](#ref-2), avait été conçu à une époque où l'imagerie du cancer de prostate reposait essentiellement sur la scintigraphie osseuse et le scanner. Depuis, le paysage a été transformé par :

- L'essor de la **TEP PSMA** (⁶⁸Ga-PSMA-11, [¹⁸F]DCFPyL, [¹⁸F]PSMA-1007)
- L'approbation du **[¹⁷⁷Lu]Lu-PSMA-617** (Pluvicto®) en thérapie [[3]](#ref-3)
- Le développement de **biomarqueurs moléculaires** (ctDNA, CTC, profils génomiques somatiques et germinaux)
- L'extension des traitements efficaces aux **stades plus précoces** de la maladie

Le PCWG4 a réuni un comité international d'experts entre 2016 et 2025, co-dirigé par Armstrong (Duke University) et Morris (Memorial Sloan Kettering), regroupant 50 auteurs dont des acteurs majeurs de l'imagerie TEP PSMA (Gafita, Hermann, Hofman, Hope, Emmett, Schöder, Kuo, Chen) [[1]](#ref-1).

---

## 1. Nouvelle terminologie : APMN/S et APMR

Le PCWG4 propose d'abandonner les termes « sensible à la castration » (CSPC/HSPC) et « résistant à la castration » (CRPC/HRPC), jugés **non centrés sur le patient**, au profit d'une nomenclature axée sur la **modulation de la voie androgénique** [[1]](#ref-1) :

| Ancienne terminologie | Nouvelle terminologie PCWG4 | Signification |
|---|---|---|
| HSPC / CSPC (hormone-/castration-sensitive) | **APMN/S** | Androgen Pathway Modulation–Naïve/Sensitive |
| CRPC / HRPC (castration-/hormone-resistant) | **APMR** | Androgen Pathway Modulator–Resistant |
...
## 2. Imagerie TEP PSMA : intégration dans le design des essais

C'est le changement le plus significatif pour la médecine nucléaire. Le PCWG4 intègre formellement la **TEP PSMA** dans le cadre des essais cliniques, avec plusieurs recommandations structurantes [[1]](#ref-1).

### 2.1 Statut de la TEP PSMA
...
Cette position est **cohérente avec le consensus SPARC** [[4]](#ref-4), qui rappelle que les variations de SUV ne sont pas un critère de progression tumorale. Elle se distingue toutefois des critères **RECIP 1.0**, qui combinent le volume tumoral PSMA (PSMA-VOL) et l'apparition de nouvelles lésions pour catégoriser la réponse. Le PCWG4 repose uniquement sur le comptage de nouvelles lésions, un choix délibéré de simplicité en attendant la validation de critères plus granulaires.

### 2.6 Validation préliminaire : l'essai PRINCE

Les critères préliminaires du PCWG4 en TEP PSMA ont été testés prospectivement dans l'essai **PRINCE** ([¹⁷⁷Lu]Lu-PSMA-617 + pembrolizumab) avec relecture en aveugle par 5 médecins nucléaires expérimentés [[5]](#ref-5). L'accord inter-observateurs pour la progression (PD vs non-PD) était quasi-parfait (κ = 0,90), et les critères PCWG4 détectaient la progression plus tôt que le PCWG3 (rPFS médiane 9,4 vs 19,9 mois), avec corrélation significative avec la survie globale. C'est à ce jour la **seule validation empirique prospective** des critères PCWG4 en TEP PSMA.

### 2.7 TEP PSMA et évaluation de la réponse
...
:::warning RC PSMA ≠ RC tumorale certaine
...
|---|---|---|
| **Néoadjuvant/adjuvant (MFS)** | Baseline, 6, 12, 18 mois | Selon PCWG3 |
| **APMN/S ± métastases (MFS/rPFS)** | Baseline, 3, 6, 12, 18 mois | Selon PCWG3 |
| **APMR ± métastases (rPFS)** | Baseline + 3 premiers examens, puis tous les 6 mois | Selon PCWG3 les 6 premiers mois, puis tous les 3 mois |

---

## 3. Critères d'éligibilité et biomarqueurs
...
### 3.3 Biomarqueurs de réponse candidats

Le PCWG4 identifie quatre biomarqueurs de réponse candidats prioritaires [[1]](#ref-1) :
...
## 4. Endpoints et design des essais

### 4.1 Extension aux stades précoces

Le PCWG4 étend ses recommandations sur les endpoints aux stades plus précoces (APMN/S) [[1]](#ref-1) :

- La **survie sans progression radiographique (rPFS)** définie par le PCWG3 a démontré une association modérée à forte avec la survie globale à travers de multiples essais de phase III et à travers une gamme d'états de la maladie
- La **survie sans métastase (MFS)** par scanner/scintigraphie/IRM est traitée par les agences réglementaires comme un événement clinique de facto
- Les deux sont recommandés comme endpoints dans les stades précoces
...
|---|---|---|
| **Seuil de variation** | Augmentation ≥ 25 % au-dessus du nadir | Tout accroissement par rapport au baseline/nadir |
| **Confirmation** | Mesure confirmée à ≥ 3 semaines | Mesure confirmée à ≥ 21 jours |
| **PSA minimum requis** | PSA absolu > 2 ng/mL | — |
| **Élévation minimale** | — | Augmentation ≥ 0,2 ng/mL par rapport au nadir |

Le PCWG4 recommande de **reporter la progression PSA selon les deux définitions** (PCWG2/3 et PCWG4) dans tous les essais, afin de permettre une comparaison entre les cadres [[1]](#ref-1).

---

## 5. PCWG4 et SPARC : complémentarité

Il est important de positionner le PCWG4 par rapport au consensus **SPARC** [[4]](#ref-4), récemment publié et détaillé dans notre [cours sur les critères d'interprétation TEP PSMA](/docs/pathologies/prostate/criteres-interpretation-tep-psma) :

| Dimension | PCWG4 [[1]](#ref-1) | SPARC [[4]](#ref-4) |
|---|---|---|
| **Objectif** | Design des essais cliniques, développement de médicaments et biomarqueurs | Reporting et interprétation standardisés de la TEP PSMA |
| **Progression** | Règle des 5 (nouvelles lésions, par site) | Pas de définition validée de progression basée sur la TEP PSMA seule |
| **Réponse** | RC uniquement (disparition de la maladie PSMA+) ; pas de RP/MS ; collecte de données pour PCWG5 | RECIP 1.0 et PPP (PSMA PET Progression) reconnus mais non standardisés |
| **SUV** | Pas de critère de progression/réponse basé sur le SUV | Les variations de SUV ne sont pas un critère de progression |
| **Scope** | Tous les stades (APMN/S et APMR) | Principalement staging et reporting |

Les deux frameworks sont **complémentaires** : SPARC guide le reporting clinique, le PCWG4 guide le design des essais [[1]](#ref-1).
...
- L'approche est **visuelle** (comptage de lésions) — les variations de SUV ne sont pas validées
- La terminologie change : **APMN/S** (ex-CSPC/HSPC) et **APMR** (ex-CRPC) — abandon du terme « castration »
- Les patients doivent être caractérisés par **génotype, imagerie, biomarqueurs et traitements antérieurs**, pas par « ligne de traitement »
- Le PCWG4 est **complémentaire du consensus SPARC** [[4]](#ref-4) : PCWG4 pour le design des essais, SPARC pour le reporting clinique
:::
...
1. <a id="ref-1"></a>Armstrong AJ, Morris MJ, Abida W, et al. Trial Design and Objectives for Patients With Prostate Cancer: Recommendations From the Prostate Cancer Working Group 4. *J Clin Oncol*. 2026. Online ahead of print.
   [PubMed](https://pubmed.ncbi.nlm.nih.gov/41744290/)

2. <a id="ref-2"></a>Scher HI, Morris MJ, Stadler WM, et al. Trial Design and Objectives for Castration-Resistant Prostate Cancer: Updated Recommendations From the Prostate Cancer Clinical Trials Working Group 3. *J Clin Oncol*. 2016;34(12):1402-1418.
   [PubMed](https://pubmed.ncbi.nlm.nih.gov/26903579/)

3. <a id="ref-3"></a>Sartor O, de Bono J, Chi KN, et al. Lutetium-177-PSMA-617 for Metastatic Castration-Resistant Prostate Cancer. *N Engl J Med*. 2021;385(12):1091-1103.
   [PubMed](https://pubmed.ncbi.nlm.nih.gov/34161051/)

4. <a id="ref-4"></a>Herrmann K, Walz J, MacLennan S, et al. SPARC: The Standardised PSMA PET/CT Analysis and Reporting Consensus: A Delphi Analysis. *Eur Urol*. 2025;88(3):282-291.
   [PubMed](https://pubmed.ncbi.nlm.nih.gov/40945999/)

5. <a id="ref-5"></a>Hofman MS, Emmett L, Sandhu S, et al. Prostate cancer working group 4 (PCWG4) preliminary criteria using serial PSMA PET/CT for response evaluation: Analysis from the PRINCE trial. *Ann Oncol*. 2024;35(Suppl 2):S970.
   [Abstract](https://www.annalsofoncology.org/article/S0923-7534(24)03208-3/fulltext)
